...
First launch: tap **API key & language**, save your key. Use the **address bar**: tap it to see **recent URLs** (and type to filter). **Go** loads the page. **Long-press a word** (or hold ~½ second) to translate. After a word translation, tap **Full sentence** to translate the whole context snippet; **Back to word** returns to the word view. The close control is HTML `&#215;` plus UTF-8-safe injection so it is not garbled.
